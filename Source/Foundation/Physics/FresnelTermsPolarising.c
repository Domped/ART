...
#define ART_MODULE_NAME     FresnelTermsPolarising
        ...
        )
{
        ...
        double me = sqrt( M_SQR( kk * gamma * qe - alpha * M_SQR( qo ) ) + M_SQR( k_2 ) * beta_2 * M_SQR( eo ) + M_SQR( kk * alpha * qe + gamma * M_SQR( qe ) - t2 * gamma ) );
        ...
        *attenuation_reflect_perpendicular = M_SQR(( (( qi - qe) * a * eey - ( qi - qo ) * b * eoy ) / f ) + ( t4 * ( a * eex - b * eox ) / f ) );
        *attenuation_reflect_parallel = M_SQR( (( 2 * qt / f ) * ( ( qi + qe) * eox * eey - ( qi + qo) * eex * eoy ) - 1 ) + ( t4 * ( qe - qo ) * eoy * eey / f) );

        *attenuation_refract_o_perpendicular = ( ( no * cos( cos_phi_o ) ) / ( ni * cos_phi) ) * M_SQR(-2 * qi * b / f);
        *attenuation_refract_o_parallel = ( ( no * cos( cos_phi_o ) ) / ( ni * cos_phi) ) * M_SQR(t4 * ( qi + qe ) * eey / f);

        *attenuation_refract_e_perpendicular = ( ( ne_var * cos( cos_phi_e ) ) / ( ni * cos_phi) ) * M_SQR(-2 * qi * a / f);
        *attenuation_refract_e_parallel = ( ( ne_var * cos( cos_phi_e ) ) / ( ni * cos_phi) ) * M_SQR(-t4 * ( qi + qo ) * eoy / f);

        //printf("%f %f %f %f %f %f %f %f %f\n", cos_phi, *attenuation_refract_o_perpendicular, *attenuation_refract_o_parallel,*attenuation_refract_e_perpendicular, *attenuation_refract_e_parallel, *attenuation_reflect_perpendicular, *attenuation_reflect_parallel, no, ne);

        *retardance_senkrecht    = - MATH_PI_DIV_2;

        if ( atan ( no ) < acos ( cos_phi ) )
                *retardance_parallel =   MATH_PI_DIV_2;
        else
                *retardance_parallel = - MATH_PI_DIV_2;
}
