...
#define ART_MODULE_NAME     ArnFisheyeCamera

#import "ArnFisheyeCamera.h"
...
@implementation ArnFisheyeCamera

ARPCONCRETECLASS_DEFAULT_IMPLEMENTATION(ArnFisheyeCamera)
        ...
        : (      Ray3D *) outRay
{
    ...
    double denom   = M_SQR(scaledx)+M_SQR(scaledy)+1;

    if ( M_SQR(scaledx) + M_SQR(scaledy) > 1.0 )
    { // outside image area
        return NO;
    }
    ...
    XC(getRayVector) = 2.0*scaledx/denom;
    YC(getRayVector) = 2.0*scaledy/denom;
    ZC(getRayVector) = -(denom-2.0)/denom;
    ...
    if ( outReferenceFrame )
    {
        ...
        vec3d_norm_v(& ref1);
        vec3d_norm_v(& ref0);
        vec3d_v_rot_v(&ref1,&rot, &ARREFFRAME_RF_I( *outReferenceFrame, 1 ));
        vec3d_v_rot_v(&ref0,&rot, &ARREFFRAME_RF_I( *outReferenceFrame, 0 ));
    }
