...
#define ART_MODULE_NAME     ArnParaboloidRayCasting
...
@implementation ArnParaboloid ( RayCasting )

ARPRAYCASTING_DEFAULT_IMPLEMENTATION(ArnParaboloid)
        ...
        : (struct ArIntersectionList *) intersectionList
{
    ...
    arnraycaster_count_test(rayCaster, ArnParaboloid);
#endif
    ...
    if (shapeGeometry & arshape_z_bounded)
    {
        if (OBJECTSPACE_RAYDIR & ray3ddir_positive_z)
        {
            if (shapeGeometry & arshape_min_z_bounded)
            {
                ...
                RAISE_MIN_T_AND_RETURN_IF_OUTSIDE_RANGE(0,t,faceindex_for_min_t,range_of_t);
            }
        ...
        else if (OBJECTSPACE_RAYDIR & ray3ddir_negative_z)
        {
            ...
            {
                double t = - ZC(OBJECTSPACE_RAY_POINT) * ZC(OBJECTSPACE_RAY_INVVEC);
                LOWER_MAX_T_AND_RETURN_IF_OUTSIDE_RANGE(0,t,faceindex_for_max_t,range_of_t);
            }
        }
        else
        {
...
#ifdef ART_WITH_INTERSECTION_STATISTICS
    arnraycaster_count_intersection(rayCaster, ArnParaboloid);
#endif
}
        ...
        : (ArcIntersection *) intersection
{
    if ( FACE_ID(intersection) == 0)
    {
        ...
        OBJECTSPACE_NORMAL_X(intersection) =   2.0 * OBJECTSPACE_POINT_X(intersection);
        OBJECTSPACE_NORMAL_Y(intersection) =   2.0 * OBJECTSPACE_POINT_Y(intersection);
        OBJECTSPACE_NORMAL_Z(intersection) = - 1.0;
